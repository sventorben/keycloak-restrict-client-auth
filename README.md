...
![Keycloak Dependency Version](https://img.shields.io/badge/Keycloak-24.0.2-blue)
...
Here is one example: suppose a user tries to log in via the built-in browser flow, at the end of which you have added this additional step. If the "Cookie" or "Forms" alternative is used, the user will proceed to the "Restrict user authentication on clients" step and be evaluated. But if it is the "Identity Provider Redirector" alternative which gets used, the subsequent steps will be skipped and the user will authenticate without this validation. This plugin must be configured separately on the identity provider's post login flow in order to apply (this is a general feature of how brokering works in Keycloak authentication flows, not specific to this plugin).
